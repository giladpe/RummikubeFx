...
import rummikub.Engine.Serie;
import rummikub.Engine.Table;
...
{
    URL csdURL = XmlUtils.class.getResource("/" + RESOURCES + "/" + SCHEMA_NAME);//get schema from resources folder
    ...
    //attach the Schema to the unmarshaller so it will use it to run validations
      ...
      Rummikub rummikub = (Rummikub) unmarshaller.unmarshal(new File(filePath));//get the game from XmlUtils file
...
{          
        ...
        URL csdURL = XmlUtils.class.getResource("/" + RESOURCES + "/" + SCHEMA_NAME);
        SchemaFactory schemaFactory = SchemaFactory.newInstance(XMLConstants.W3C_XML_SCHEMA_NS_URI);
        ...
        marshaller.marshal(rummikub,os);//save to XmlUtils file in the choosen path
...
{    
 Players xmlPlayer = new Players();//create type player from XmlUtils generated classes
 
    for (Player player : game.getPlayers()) {//go over all the player from XmlUtils file
    ...
    saveGame(rummikub,FilePath);//go save the game in XmlUtils after we finish to convert fields
}
////////////////////////////////////////////////////////////////////////////////
public static void  createBoardForXml(Rummikub rummikub,Table table){
  ...
  for(Serie serie : table.getSerieList()){//create board to save for XmlUtils file
...
public static void getTilesForXmlSerie( XmlClasses.Board.Sequence sequence,Serie serie){
   ...
   XmlClasses.Tile Xmltile = new XmlClasses.Tile();
    ...
    for(Tile tile : serie.getSerie())//get the tiles for series
    {
        ...
        Xmltile.setValue(tile.getValue());//then convert them to XmlUtils generated class tile
