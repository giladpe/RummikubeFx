    ...
    @FXML protected void handleGameNameTextChange(ActionEvent event) {
       ...
       //* starts with dont works
        if (!(gameNameString.isEmpty() || gameNameString.startsWith("\\s"))) {
            ...
            players.stream().forEach((player) -> {player.setDisable(true);
...
//    }
    ...
    @FXML protected void handleBackToMenuButtonAction(ActionEvent event) {
        
    }
    
    @FXML private void handleNumberOfPlayersChoiceChange(ActionEvent event) {
        hidePlayersFields(true);
    }
    
    @FXML private void hidePlayersFields(boolean value) {
        for (int i = 0; i < this.numberOfPlayers.getValue(); i++) {
            this.players.get(i).setVisible(value);
        }
    }
